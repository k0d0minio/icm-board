...
There is no Status row and no status vocabulary. Where a file sits — and what exists
around it — is the state:
...
`main` via the GitHub API and parses leniently — a malformed stub still appears rather
than vanishing:
