...
.icm/
  ...
  _shared/
    ...
    project-rules.md         ← what is true of THIS repo: people, factory, reporting, support,
                                and the learned rules retrospective.sh appends                (P)
  ...
  scripts/
    ...
    deploy-status.sh rollback.sh security-check.sh health-check.sh
    usage-snapshot.sh env.sh setup.sh retrospective.sh                                    (T)
...
| `env-check.sh [--fix]` | pre-flight: binaries, a GitHub route, `required_env`, `complexity`, the folder shape, executable bits (repaired only with `--fix`), a UTF-8 locale, the security utilities (gitleaks, the lockfile's audit tool, a declared `health_endpoint` — reported, never required) | `PASS` 0 · `FAIL` 1 |
...
| `security-check.sh [<slug>] [--base <ref>] [--all] [--no-audit]` | Build step 9 and Release step 4 — stop class 2, **measured**: the lines this branch added, scanned for secrets (gitleaks when installed, the repo's `.gitleaks.toml` honoured; the built-in patterns otherwise — a floor), and the lockfile's own audit at `high` on every call (`pnpm`/`npm`/`yarn audit`, `cargo audit`, `pip-audit`; a missing tool or an unreachable registry is SKIPPED, never a finding; `--no-audit` is the operator's recorded waiver). Findings — `file:line` and the rule, **never the value** — appended to `runs/<slug>/03_build/output/error.log`; installs, fixes and edits nothing | `CLEAN` 0 · `FINDINGS n` 1 · `SKIP` 0 |
