...
| [scripts/vercel-env.sh](scripts/vercel-env.sh) | The estate's Vercel env plumbing, over [scripts/vercel-env-registry.json](scripts/vercel-env-registry.json) — which repo/app path is which Vercel project, on which of the three teams. `link`, `init` and `pull` so far — `init` seeds each app's committed `.env.example` from the names Vercel holds, never values, never overwriting a line, and `pull` writes each app's `.env.local` from Vercel's development environment with those notes interleaved above the keys; notes, values and docs each flow one way (epic `vercel-env-system`). Local machine only, and per-team `VERCEL_TOKEN_*` env vars only. |
