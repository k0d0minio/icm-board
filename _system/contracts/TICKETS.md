...
- Prefixes: `ICM` (icm-board), `REMI`, `AGORA`, `BERC`, `KAU`, `VINE`, `JN`
  (jamienisbet), `BARZ` (barzinho), `BTM` (boystomenretreat), `COLL` (collabimmo),
  `CASEY` (casey-hebbel), `CAFE` (cafe-jardim), `MESSY` (messy-play), `DND`
  (dungeons-dragons), `LIB` (the-library); new repos pick a short unique prefix. Repos
  seeded by `icm-check.sh --fix` carry an auto-derived prefix in their intake README —
  confirm it (and register it here + in `icm-check.sh`'s map) before cutting the first
  ticket.
