...
None of them is ever called by a repo: they are the estate's cross-repo tools and run from
this checkout on Jamie's machine (agency brief flag 1 — nothing a repo runs depends on
seeing icm-board).
...
| [scripts/tickets-board.sh](scripts/tickets-board.sh) | The estate board. `--today` powers the SessionStart hook. Like `ticket-hygiene.sh`, it reads each client repo at its **ticket base branch** (`origin/<base>`, D38) through [scripts/lib/ticket-base.sh](scripts/lib/ticket-base.sh) — never the shared checkout, which lags `uat`. |
...
.claude/
  ...
  hooks/ · skills/   ← canonical estate assets; drift reported, repo wins
                       (skills/pipeline and skills/setup route the pipeline)
...
- **Redirect files, not copies**, for cross-layer references — except where a repo must
  ...
  micro-copy.
- **Ticket state goes straight to `main`, in every repo** — `Plan:`/`Wrap:`/`Scope:`
  commits and stub moves, paths staged explicitly; code goes through a PR on a `claude/`
  branch that the operator merges. `main` is the only long-lived branch anywhere, so there
  is nothing to drift (D39; the canonical `pr-conventions` skill). icm-board's `Deal:`
  commits take the same lane.
