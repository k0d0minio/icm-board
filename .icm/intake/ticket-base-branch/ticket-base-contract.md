...
1. **Register:** add the next free D-number (D37 at cutting) to `.icm/project.md` — the decision
   ...
   the session merging it immediately (no check wait; path guard verified first; `--admin` where a ruleset requires checks), Scope's front as a ticket PR, hotfix + knowledge lane on `main` with
   `sync` required after both, icm-board exempt. It amends D31 and PR regime 1.
2. **Contracts:** `_system/contracts/TICKETS.md`, `_system/contracts/PIPELINE.md` (front, UAT
...
3. **Template (T files):** `_shared/github.md` regime 1 rewritten (front = ticket PR),
...
4. **Canonical skills, both copies** (`.claude/skills/` and `_system/template/claude/skills/`):
   `pr-conventions` (the ticket PR shape, the path-guard check, the immediate merge — the one PR an
   agent merges; icm-board's copy says the direct-commit exemption is icm-board only) and `ticket-craft` ("the board reads the ticket base branch — a stub exists
   once its PR merges").
5. **icm-board's delivery stages:** `workspaces/deliver/stages/day/CONTEXT.md` step 5 + Outputs,
...
- [ ] `pr-conventions` states the merge rule once: verify the path guard, then merge at once
      without waiting for checks; stop and never merge a PR that touches anything outside it
...
decision and the full file list. Record the decision under the next free D-number in
