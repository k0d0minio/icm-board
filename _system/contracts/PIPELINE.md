...
on every merge with its own variables and a **named** database (`database.neon.uat_branch` /
`database.mongodb.uat_name`), `url` the domain attached to it — the client's one fixed address.
